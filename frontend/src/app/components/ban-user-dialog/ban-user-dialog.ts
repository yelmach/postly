...
export class BanUserDialog {
  ...
  private handleError(error: HttpErrorResponse) {
    if (error.status === 400) {
      if (error.error?.message?.includes('already banned')) {
        this.formError.set('This user is already banned');
      } else if (error.error?.message?.includes('cannot ban admin')) {
        this.formError.set('Cannot ban admin users');
      } else if (error.error?.message?.includes('cannot ban yourself')) {
        this.formError.set('You cannot ban yourself');

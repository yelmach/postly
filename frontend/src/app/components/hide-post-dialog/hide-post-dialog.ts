...
export class HidePostDialog {
  ...
  private handleError(error: HttpErrorResponse) {
    if (error.status === 400) {
      if (error.error?.message?.includes('already hidden')) {
        this.formError.set('This post is already hidden');
      ...
      this.formError.set('You do not have permission to hide posts');
    } else if (error.status === 404) {
      this.formError.set('Post not found');
    } else if (error.error?.message) {
      this.formError.set(error.error.message);
    } else {

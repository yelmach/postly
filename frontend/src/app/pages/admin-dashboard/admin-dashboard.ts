...
export class AdminDashboard implements OnInit {
  ...
  onUserRowClick(user: AdminUser, event: MouseEvent) {
    // Don't open dialog if clicking on action button/menu
  ...
  onDeleteUser(user: AdminUser, event?: MouseEvent) {
    ...
    if (
      confirm(
        `Are you sure you want to delete @${user.username}? This action cannot be undone and will delete all their posts and data.`
      )
    ) {
  ...
  onPostRowClick(post: AdminPost, event: MouseEvent) {
    // Don't open post if clicking on action button/menu
    const target = event.target as HTMLElement;
    if (target.closest('button') || target.closest('.mat-mdc-menu-trigger') || target.closest('a')) {
      return;
    }
  ...
  onViewReportDetails(report: ReportResponse) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result?.success) {
        ...
        this.loadReports();
        this.loadStats();
        // Reload users/posts if moderation action was taken
